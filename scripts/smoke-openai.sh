...
#
# End-to-end smoke test for the OpenAI enricher backend. Mirror of
# scripts/smoke-anthropic.sh — same skip semantics, same idempotency
# assertion.
#
# Behavior:
#   - When OPENAI_API_KEY is unset → exits 0 with a skip message. This
...
#     ships this script can't drive the openai path through the CLI.
#   - When both prerequisites are present → runs `dashgen generate
#     --provider openai --fixture-dir testdata/fixtures/service-basic
...
if [[ -z "${OPENAI_API_KEY:-}" ]]; then
    echo "[smoke-openai] OPENAI_API_KEY unset; skipping smoke." >&2
    exit 0
fi
...
if ! go run ./cmd/dashgen generate --help 2>&1 | grep -q -- '--provider'; then
    echo "[smoke-openai] --provider flag not yet wired in cmd/dashgen (v0.2 Step 4.2); skipping CLI smoke." >&2
    exit 0
fi
...
echo "[smoke-openai] First run (cold cache, real OpenAI API call)..."
...
echo "[smoke-openai] Second run (must hit cache and produce byte-identical output)..."
go run ./cmd/dashgen generate \
    --provider openai \
    --fixture-dir testdata/fixtures/service-basic \
    --profile service \
...
echo "[smoke-openai] Diffing outputs..."
fail=0
for f in dashboard.json rationale.md warnings.json; do
    if ! cmp -s "$OUT1/$f" "$OUT2/$f"; then
        echo "[smoke-openai] FAIL: $f differs between runs (cache miss or non-determinism)" >&2
...
if (( fail )); then
    echo "[smoke-openai] FAIL: idempotency violated; AI off-parity contract broken." >&2
    exit 1
fi

echo "[smoke-openai] PASS: outputs are byte-identical across runs."
