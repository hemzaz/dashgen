...
> Status: SHIPPED (v0.3.0, 2026-04-29). All phases (0/1A/1B/2A/2B/3/4A/4B/5/6A/6B/7/8) closed.
> v0.3.0 tagged + binaries published. Preserved here as the execution
> record (per-phase task plan, DoD, team assignments) — useful for
> archeology and as a template for future major migrations. Forward
> work tracks in [`V0.4-QUEUE.md`](V0.4-QUEUE.md) and
> [`BIG_ROCKS.md`](BIG_ROCKS.md).
>
> Source specs (current; not duplicated here):
...
| 8 | v0.3.0 Release + Distribution | yes | 3 (writer + executor + verifier) | 1.5–2 days |
...
### Phase 8 — v0.3.0 Release + Distribution

**Goal:** Doc updates, release notes, distribution artifacts (binaries + install path), tag, push.

**Depends on:** Phase 7 complete.

**Why distribution is a hard gate (not a follow-up):** v0.2.0 shipped without binaries on the GitHub release page; the install path is `go install` only, which filters out users without a Go toolchain. v0.3.0 ships the user-extensibility story (drop YAML in `~/.config/dashgen/recipes/`, run dashgen) — but that story has no audience if users can't install dashgen in the first place. Distribution work that lands at v0.3.0-tag time is the difference between "an extensible tool you have to compile" and "an extensible tool you can use."

**OMC team:** `team v0-3-phase-8`, 3 workers.
...
##### T8.2 — Distribution setup (goreleaser + binaries + install path)
- **Owner:** executor
- **Model:** sonnet
- **Depends on:** T8.1 (CHANGELOG must exist; goreleaser uses it)
- **Scope:**
  - In: `.goreleaser.yaml` at repo root configured for darwin-amd64, darwin-arm64, linux-amd64, linux-arm64. Build invokes `go build ./cmd/dashgen` with `-trimpath` + `-ldflags="-s -w"` for reproducible builds.
  - In: GitHub Actions release workflow at `.github/workflows/release.yml` that triggers on tag push (`v*`), runs `goreleaser release`, attaches built archives + checksums to the GH release page. Honors `GITHUB_TOKEN` (default scope is sufficient).
  - In: `README.md` "Install" section refresh: keep the `go install` path, add a "Download a binary" path with copy-pasteable `curl` commands per platform pointing at the GH release URL. Cross-link to the release page.
  - In: a `scripts/release-dryrun.sh` that runs `goreleaser release --snapshot --clean` locally so the release path can be exercised without a real tag. Produces `dist/` artifacts for spot-checking.
  - In: README.md note: "macOS binaries are unsigned in v0.3.x; users will see a Gatekeeper prompt. Notarization tracked for v0.4." (Notarization is genuinely out of scope — Apple Developer ID + Apple-Notary stack is its own multi-day adventure that shouldn't gate v0.3.0.)
  - Out: macOS code signing / notarization (deferred).
  - Out: Homebrew tap, Snap, Docker images (deferred — track as v0.4 candidates).
  - Out: actually triggering a release (`goreleaser release` runs only on tag-push in CI; T8.3 verifier runs the snapshot dry-run as a check).
- **DoD:**
  - **Functional:** `bash scripts/release-dryrun.sh` produces `dist/dashgen_*_darwin_amd64.tar.gz` + arm64 + linux × 2 + `checksums.txt`. Each archive contains a working `dashgen` binary (`./dist/.../dashgen --help` prints help).
  - **Functional:** README.md install section names a verifiable URL pattern for the binary download (even if the v0.3.0 release doesn't exist yet at doc-write time).
  - **Quality:** `.goreleaser.yaml` validates with `goreleaser check`.
  - **Quality:** the GH Actions workflow lints clean (`actionlint` or equivalent if available).
  - **Integration:** existing `Makefile` build path is unaffected; `go install ./cmd/dashgen` continues to work.

##### T8.3 — Verifier sign-off + tag annotation
- **Owner:** verifier
- **Model:** sonnet
...
- **Scope:**
  ...
  - In: run `bash scripts/release-dryrun.sh` and confirm all four binary archives + checksums are produced.
  - In: produce the v0.3.0 tag annotation message file (with a one-line "Binaries available on the release page" pointer).
  - Out: running `git tag` (the user's decision to tag and push).
- **DoD:**
  - **Functional:** verification report GREEN; release-dryrun produces all four binaries.
...
- [ ] `.goreleaser.yaml` + release.yml workflow committed.
- [ ] `bash scripts/release-dryrun.sh` produces 4 binaries + checksums.
- [ ] README install section names the binary-download path.
- [ ] Verifier GREEN.
- [ ] Tag annotation prepared.
- [ ] **User decides** whether to tag + push. On tag-push, the release workflow runs and attaches binaries to the GH release page automatically. **No manual `goreleaser release` invocation by the team — that's the user's authorization.**
...
- [ ] **Distribution: `.goreleaser.yaml` + GH release workflow committed; `release-dryrun.sh` produces darwin-amd64 + darwin-arm64 + linux-amd64 + linux-arm64 archives + checksums; README "Install" section names the binary-download path.**
- [ ] Tag annotation prepared (user decides to tag + push; tag-push triggers the release workflow which attaches binaries automatically).
...
## 9. Phase 0 Outcomes & Carry-forward

Phase 0 closed 2026-04-27 with verdict **GREEN** (Phase 1A unblocked). Deliverables:

- `internal/recipes/schema.cue` (commits `f917c01` + `11c0218`)
- `internal/recipes/testdata/valid/*.yaml` (7 example recipes)
- `docs/RECIPES-DSL-HELPERS.md` (commit `712cf91`) — 5 shipped helpers, 5 deferred per closed-namespace policy, 9 banned helpers
- `docs/V0.3-PHASE-0-REVIEW.md` (commit `162d808`) — full threat-coverage matrix, Tier-C stress test (5/5 unify), slope audit
- `internal/recipes/testdata/migration_tiers.yaml` (commit `f91f52a`) — 47 entries, 3 split parents, all source paths verified

### Phase 0 surfaced — carry-forward items

These don't block Phase 1A but must be addressed before Phase 8 release prep:

1. **DSL §4 / §4.5 prose drift.** Inline CUE snippets in `RECIPES-DSL.md` §4.1 and §4.5 still depict `name`/`section`/`profile`/`confidence`/`tier` at the top level of `#Recipe`. The actual `schema.cue` and the wire format (§5.1) nest them under `metadata: #Metadata`. Fix: rewrite the §4 / §4.5 snippets to match. Surfaced by T0.3 review (§3 slope audit).
2. **`infra_network` split decision.** §12.1's inline note says "single-recipe-two-panel for parity" but that yields 46 entries, not the planned 47. T0.4 (worker-3) elected to split into `infra_network_receive` + `infra_network_transmit` and set `expected_golden_delta: regen`, with a `notes:` field on the YAML entry flagging it for confirmation. The migration_tiers.yaml is canonical going forward; `RECIPES-DSL.md §12.1` should be updated to match (split, regen) before Phase 5.
3. **Tier-count reconciliation.** §12.2 phase-budget counts (A=10, B=12, C=22) diverge from the row-level labels in §12.1 (A=9, B=15, C=20). Both sum to 44 originals, so the totals are consistent — the per-tier breakdown isn't. **`migration_tiers.yaml` is now the authoritative source** for which recipe lives in which tier. Phase 4A/5/6A workers should key off the YAML, not the §12.2 budget. The §12.2 budget figures should be normalized to the YAML before Phase 8.
4. **`#MetricNameASCII` regex non-printable bytes (W1).** Current regex `=~"^[\\x00-\\x7f]+$"` accepts BEL (\\x07), NUL (\\x00), DEL (\\x7f). Prometheus naming doesn't exercise control bytes in practice, but Phase 1A T1A.1 should tighten to printable-ASCII (`[\\x20-\\x7e]`) when wiring the loader. Documented in `V0.3-PHASE-0-REVIEW.md` §5.

### Open follow-up (filed during Phase 0 team)

- Task `#5` (filed by worker-2 in the Phase-0 team task list) covers item 1 above. The team is being torn down; the task content is preserved verbatim in this section.

---

## 10. Phase 1A Outcomes & Carry-forward

Phase 1A closed 2026-04-27. Six commits across three workers; full suite passes 738 tests under `-race`. Loader/matcher/template/pair/registry stack is end-to-end functional and the existing Go recipe tests still green (zero regressions).

### Deliverables

| Commit | Subject | Worker |
|---|---|---|
| `a703ae4` | feat(recipes): add CUE-validated YAML loader skeleton (T1A.1) | worker-1 |
| `bc9570d` | feat(recipes): add match-predicate evaluator (T1A.2) | worker-2 |
| _T1A.3_ | feat(recipes): add text/template engine + helper FuncMap (T1A.3) | worker-3 |
| `6dbd3c8` | feat(recipes): add pair resolver (T1A.4) | worker-2 |
| `e5a0157` | feat(recipes): add YAMLRecipe + registry merge (T1A.5) | worker-1 |
| _T1A.6_ | test(recipes): parameterized YAML recipe testing harness (T1A.6) | worker-3 |

Files added: `loader.go` (1000 lines), `schema_embed.go` (18), `matcher.go`, `template.go`, `pair.go`, `yaml_recipe.go` (350), extended `registry.go` (+202), plus their test files. `cuelang.org/go` pinned at v0.16.1 (matches the Phase 0 `cue` CLI).

### Phase 1A surfaced — carry-forward items

These do not block Phase 2A or Phase 3 but DO have implications for **Phase 1B (first 5 migrations)**, which is the next critical-path step:

5. **ScopeFilter is empty in YAML BuildPanels (BLOCKS Phase 1B golden parity).** YAMLRecipe.BuildPanels does NOT thread a scope filter into RenderContext.ScopeFilter — templates that contain `{ {{ .ScopeFilter }} }` render as `{ }` (technically valid PromQL with empty matchers, but semantically different from what Go recipes produce today). Existing Go recipes inject `job=~".+", instance=~".+"` (or similar) via synth-layer scoping. **Phase 1B's T1B.1 MUST include scope-filter wiring as a prerequisite to golden parity** — otherwise the goldens will diverge and the migration will fail its byte-identical contract. Recommended: extend `RenderContext` with a `ScopeFilter string` populated from the synth layer's existing scope context, and update each of the 7 `testdata/valid/*.yaml` recipes to interpolate it correctly. T1A.1 + T1A.5 surfaced this together; worker-1's T1A.5 completion message §5 documents it.

6. **CUE silently fills `apiVersion` (loader-side defensive check).** The `apiVersion: "dashgen.io/v1"` exact-match constraint in the schema does NOT actually reject YAMLs that omit the field — CUE unification fills it in. Worker-1's loader added a raw-yaml-map defensive check before unification to enforce the requirement at decode time. This belongs in the schema as `apiVersion!: "dashgen.io/v1"` (CUE required-field syntax) rather than as a loader-side guard, BUT the existing defensive check is sufficient for v0.3.0; tracked as a Phase 8 schema cleanup.

7. **Panel kind coercion is lossy.** YAMLRecipe maps `gauge → ir.PanelKindStat` and `barchart → ir.PanelKindGraph` because `ir.PanelKind` only has `stat`/`graph`/`timeseries`. None of the 47 migration_tiers entries use `gauge` or `barchart`, so this is a no-op for v0.3.0, but if a future recipe needs them, ir must grow new kinds first. Tracked in `internal/recipes/yaml_recipe.go:panelKindFor`.

8. **Pair `WarnOnMissing` does not yet emit `WarningPairMissing`.** YAMLRecipe.BuildPanels honors `OmitOnMissing` (skip metric) and treats `WarnOnMissing` / `UseFirstOnly` identically (skip pair-required panels, emit non-pair panels). The DSL §8.2 spec says `warn` should produce a refused-candidate panel with `WarningPairMissing`. That's a Phase 1B+ wiring task (touches synth/validate, not the recipe layer). Tracked.

9. **Regex caching via package-level sync.Map.** worker-2 added a `sync.Map` cache populated by `matcher.ValidateBudget` at decode time so each `name_matches` regex compiles once. This is a clean workaround for the schema not pre-compiling into the struct; documented in matcher.go.

10. **Heavy transitive deps from `cuelang.org/go`.** go.sum grew by 175 lines (Azure/AWS/GCP SDKs via OCI registry support). Cobra incidentally upgraded v1.8.1 → v1.10.2 by `go mod tidy`. Builds + tests still clean. Tracked for v0.4 if the transitive surface causes friction; for v0.3 it's accepted.

### Phase 1B launch instructions (preview)

Before launching Phase 1B's team, the team-lead must add scope-filter wiring as an **explicit prerequisite task** in T1B.1. Suggested scope addition: "Before migrating any recipe, extend `RenderContext` with a `ScopeFilter string` field, thread it from synth's per-recipe scope context into `YAMLRecipe.BuildPanels`, and update every `testdata/valid/*.yaml` query_template that uses `{ {{ .ScopeFilter }} }` to render the same scope filter Go recipes produce today. Verify byte-identical goldens against the existing 6 fixtures before declaring T1B.1 complete."

Failure to wire scope filter first will produce diverging goldens, and Phase 1B's byte-identical-output contract will break.

> **Correction (2026-04-27):** Phase 1B execution verified that the inline italics above ("the same scope filter Go recipes produce today") was wrong. Worker-1's audit of all 19 service / 13 infra / 12 k8s Go recipes found **zero** scope-filter injection. Every Go recipe emits `rate(metric_name[5m])` / `max by (...) (metric_name)` etc. without any matcher block. The threading wiring landed (commit `4d3ba05`) carries an empty `ScopeFilter` string end-to-end — that's the correct shape for byte identity against today's Go output. The infrastructure is forward-compatible: synth can populate it later when v0.4+ adds dashboard-variable scoping. **Item #5 above is preserved as-written for the historical record but the parenthetical claim about Go's scope filter is false.**

---

## 11. Phase 1B Outcomes & Carry-forward

Phase 1B closed 2026-04-27 with verdict **GREEN** (Phase 2A unblocked). 746/746 tests pass under `-race`; all 6 golden suites byte-identical to v0.2.0; no goldens regenerated.

### Deliverables

| Commit | Subject | Worker |
|---|---|---|
| `4d3ba05` | feat(recipes): thread scope filter into RenderContext (T1B.1 prereq) | worker-1 |
| `a0ed206` | feat(recipes): migrate 5 representative recipes to YAML (T1B.1) | worker-1 |

5 YAML recipes registered, 5 Go recipes unregistered (files retained per Phase 4A's deletion responsibility). Each YAML has a sibling `.testdata.json` fixture (≥2 positives, ≥2 negatives, ≥1 expected_panel).

### Recipe slate change (decision recorded)

The original Phase 1B slate (per Phase-1A planning) was meant to exercise tier coverage:
- service_http_rate (Tier A)
- service_http_latency (Tier B histogram-trio)
- service_request_size (Tier B label predicate)
- infra_filesystem_usage (Tier C pair_with)
- service_gc_pause (Tier C type-dispatch)

Worker-1 escalated mid-task: 4 of these 5 require engine extensions the v0.3 schema doesn't yet support (quantile fan-out for histogram trios, hand-coded inline filters for filesystem usage, type-dispatch panel grouping for gc_pause). Byte-identical migration of any of those four cannot succeed without engine work that belongs in their respective tier phases.

**Decision:** Phase 1B's purpose is to prove the **migration loop** (load → register → byte-identical goldens), not tier coverage. Tier coverage is the responsibility of the dedicated tier phases (4A, 5, 6A), each of which owns the engine extensions its tier needs. The 5 recipes were swapped for Tier-A-only:

1. **service_grpc_rate** (service / traffic; type+trait `service_grpc`)
2. **service_goroutines** (service / saturation; name_equals `go_goroutines`)
3. **infra_cpu** (infra / cpu; name_equals + group_by mode breakdown)
4. **infra_ntp_offset** (infra / overview; name_equals `node_timex_offset_seconds`)
5. **k8s_pod_health** (k8s / pods; name_equals `kube_pod_status_phase`)

This still exercises 3 profile registries + both type+trait and name_equals matchers + grouping overrides. No engine changes required. Migration succeeded byte-identically.

### Phase 1B surfaced — carry-forward items

11. **rationale_template panel field** added to schema + loader + YAMLRecipe (~30 LOC; commit `a0ed206`). Opt-in: recipes without it fall back to the existing auto-rationale (`"YAML recipe %s (%s); metric %q ..."`). Required for Go-recipe byte-identity because Go recipes hand-write Rationale strings that the auto-template doesn't match. **Update `docs/RECIPES-DSL.md` §4 panel-template section** to document this field before Phase 8 release prep.

12. **ClassifiedInventorySnapshot.ScopeFilter field** added (commit `4d3ba05`). Synth populates it with `""` (Go-recipe parity); YAMLRecipe.BuildPanels reads it into RenderContext. Forward-compatible — synth can populate non-empty scope filters in a future v0.4+ when dashboard-variable scoping arrives. The Recipe interface itself is unchanged.

13. **Phase 4A scope drop.** Phase 4A was budgeted for "all 10 Tier-A recipes" per the §12.2 phase budget. Phase 1B consumed 5 of the 9 actual Tier-A recipes (per migration_tiers.yaml row labels). Phase 4A's residual scope is now **4 recipes**: service_http_rate, infra_load, infra_interrupts, k8s_restarts. Update §12.2 phase budgets and Phase 4A's task description before launching it.

14. **Phase 5 prerequisite — `combine_quantiles` engine extension.** Tier-B histogram-trio recipes (service_http_latency, service_request_size, service_gc_pause's histogram path) emit ONE panel containing N QueryCandidates [p50, p95, p99]. YAMLRecipe.BuildPanels currently emits ONE panel per quantile iteration. Migration of these recipes will require a `combine_quantiles: true` panel-shape flag (or equivalent) plus the BuildPanels logic to collect quantile renderings into a single panel's Queries slice instead of appending separate panels. **Track as an explicit Phase-5 prerequisite task** — Tier-B histogram migrations cannot start before this lands.

15. **Phase 6A prerequisite — pair-with engine extensions.** infra_filesystem_usage's Go recipe embeds an inline `{fstype!=""}` matcher and uses literal `{{instance}}` legend templates (Grafana-side, not Go-side template). Neither pattern is currently expressible in the YAML schema. Migration of pair-with recipes will require either a `inline_matchers` schema field OR template-escape syntax for Grafana-vs-Go template disambiguation. **Track as an explicit Phase-6A prerequisite task.**

### Open follow-up

- §12.1 / §12.2 in RECIPES-DSL.md need updating to reflect: (a) Phase 1B's actual recipe slate, (b) Phase 4A's reduced scope (4 recipes), (c) Phase 5's combine_quantiles prereq, (d) Phase 6A's inline-matcher / template-escape prereqs. Tracked for Phase 8 release prep along with §10 #1 (DSL §4 / §4.5 prose drift).

### Phase 2A launch instructions

Phase 2A (CLI skeleton: `dashgen recipe init` + `dashgen recipe scaffold`) is **unblocked**. Per V0.3-PLAN §3 Phase 2A spec: 1 executor (sonnet), 2 tasks, ~1 day. No dependencies on Phase 1B's recipe migrations — only on Phase 0's schema. Launch when ready.
...
| 2026-04-27 | Phase 0 close-out | Add §9 Phase 0 Outcomes & Carry-forward (GREEN verdict, 4 carry-forward items, deliverables list). |
| 2026-04-27 | Phase 8 distribution hard-gate | Restructure Phase 8 around three tasks (T8.1 docs, T8.2 distribution setup, T8.3 verifier sign-off). Add `.goreleaser.yaml` + GH release workflow + binary-download README path as release-blocking acceptance criteria. Rationale recorded in Phase 8 prelude. |
| 2026-04-27 | Phase 1A close-out | Add §10 Phase 1A Outcomes & Carry-forward (738 tests pass, 6 commits, loader/matcher/template/pair/registry stack landed). Six carry-forward items including the **ScopeFilter wiring gap that blocks Phase 1B golden parity** and must be the first task of T1B.1. |
| 2026-04-27 | Phase 1B close-out | Add §11 Phase 1B Outcomes & Carry-forward (746 tests pass, 2 commits, 5 Tier-A recipes migrated byte-identically). Recipe slate course-corrected mid-phase (engine extensions for Tier-B/C deferred to their tier phases). Five new carry-forward items (#11-15) including the `rationale_template` schema addition, ClassifiedInventorySnapshot.ScopeFilter field, Phase 4A scope drop to 4 recipes, and combine_quantiles / inline_matchers engine prereqs for Phases 5 / 6A. Correction added to §10 #5 about Go-recipe scope-filter injection (the original claim was wrong). |
