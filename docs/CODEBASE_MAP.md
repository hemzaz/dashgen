...
> deterministic core is shipped (v0.1); v0.2 expands the recipe catalog and
> plumbs an optional AI-enrichment seam that never touches PromQL. This
> map captures the runtime pipeline, the recipe catalog, the test fixture
> system, and the contract docs.
...
3. **Stable UIDs** — `ids.DashboardUID` hashes `(profile, inventoryHash)`; `ids.PanelUID` hashes `(dashboardUID, section, metricName, kind)`. SHA-256[:16].
...
`internal/enrich` is **wired into `generate.Run`** (commit `fa2f8f9`, "v0.2 step 0.5"). The seam is non-bypassable and determinism-preserving in the default (Noop) configuration.
...
**Cache** — on-disk; key = `(InventoryHash, Function, ProviderID, PromptHash, DashgenVersion)`. Atomic `Put` via `CreateTemp` + `Rename`; corrupt entries self-heal on next write.

**Boundary still enforced:** Input types carry only metric names + label *names*, never label values. `MetricBrief.Labels` is documented as "sorted label NAMES only; never label values" (`internal/enrich/enricher.go:66`). Per V0.2-PLAN §2.5.
