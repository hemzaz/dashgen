...
Both providers share defaults: 1024 max tokens, 30s HTTP timeout, 1 retry (10ms backoff) on 429/5xx. Missing API key returns `ErrAnthropicNoAPIKey` / `ErrOpenAINoAPIKey` at construction (NOT `ErrNotImplementedYet`); both are wrapped as `ErrBackend` by `Run`. `ollama` remains an `ErrNotImplementedYet` placeholder, deferred post-v0.3.
