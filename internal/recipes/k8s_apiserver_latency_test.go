...
func TestK8sApiserverLatency_Match(t *testing.T) {
	r := NewK8sApiserverLatency()
	cases := []struct {
		name    string
		metric  string
		...
		{
			// Different name — common app-level HTTP duration histogram that
			// must not be claimed by the k8s-specific recipe.
...
func TestK8sApiserverLatency_BuildPanels(t *testing.T) {
	...
	t.Run("k8s_profile_produces_panel", func(t *testing.T) {
		...
		for i, q := range panel.Queries {
			...
			if !strings.Contains(q.Expr, "apiserver_request_duration_seconds_bucket") {
			...
			if !strings.Contains(q.Expr, "verb") {
				t.Errorf("query[%d] must group by verb, got %q", i, q.Expr)
			}
			if !strings.Contains(q.Expr, "resource") {
				t.Errorf("query[%d] must group by resource, got %q", i, q.Expr)
			}
		}
